...
namespace Bastet {
	...
	struct HighScore {
		int			Score;
		std::string Scorer;
		bool operator<(const HighScore & b) const { return Score < b.Score; }
	...
	extern const size_t
		HowManyHighScores;	// how many high scores to save in the file
	...
	class HighScores
		: public std::vector<HighScore> {  // a set would not do the right job
										   // (think to ties)
	   public:
