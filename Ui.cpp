...
namespace Bastet {
	...
	BorderedWindow::operator WINDOW *() { return _window; }
	...
	Curses::Curses() {
		...
		if (!has_colors()) {
			endwin();
			fprintf(stderr,
					"bastet: no color support, sorry. Ask the author for a "
	...
	void Ui::DropBlock(BlockType b, Well * w) {
		...
		while (1) {	 // break = tetromino locked
			...
			if (sel_ret == 0) {	 // timeout
				if (!p.MoveIfPossible(Down, b, w)) break;
				time.tv_sec	 = 0;
				time.tv_usec = delay[_level];
			} else {  // keypress
				int ch = getch();
	...
	void Ui::CustomizeKeys() {
		...
		keys->RotateCCW
			= KeyDialog(str(fmt % "rotate tetromino COUNTERCLOCKWISE"));
		keys->Drop	= KeyDialog(str(
			 fmt
			 % "DROP tetromino (move down as much as possible immediately)"));
